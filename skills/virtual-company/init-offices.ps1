...
if (Test-Path $StatePath) {
    $ExistingOffices = [System.IO.File]::ReadAllText($StatePath, [System.Text.Encoding]::UTF8) | ConvertFrom-Json
}
...
foreach ($Member in $AllMembers) {
    ...
    if ($ExistingOffices.$OfficeName -and -not $Force) {
        Write-Host "  跳过（已存在）: $OfficeName" -ForegroundColor DarkGray
        $SkippedCount++
    ...
    try {
        ...
        Write-Host "  [OK] 创建: $OfficeName" -ForegroundColor Green
        Write-Host "       Session: $SessionId" -ForegroundColor DarkGray
        Write-Host "       模型: $($Member.Model)" -ForegroundColor DarkGray
        
    } catch {
        Write-Host "  [FAIL] 失败: $OfficeName - $_" -ForegroundColor Red
        $ErrorCount++
    }
}

# 保存状态 - 使用UTF8编码
$jsonContent = $OfficeState | ConvertTo-Json -Depth 10
[System.IO.File]::WriteAllText($StatePath, $jsonContent, [System.Text.Encoding]::UTF8)
